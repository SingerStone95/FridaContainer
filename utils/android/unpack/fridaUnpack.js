...
function unpack_common() {
    ...
    exportMethods.forEach(function (expmthd) {
        if (expmthd.name.indexOf("OpenCommon") > -1 || expmthd.name.indexOf("OpenMemory") > -1) {
            DMLog.i('dump_dex_common',"unpack_common: " + JSON.stringify(expmthd));
    ...
    if (Java.available) {
        Java.perform(function () {
            ...
            if (application != undefined) {
                DMLog.e('dump_dex_common',application);
                application.attach.overload('android.content.Context').implementation = function (context) {
                    DMLog.e('dump_dex_common',"android.content.Context");
                    var result = this.attach(context);
                    var classloader = context.getClassLoader();
                    var filesDir = context.getFilesDir();
                    var codeCacheDir = context.getCodeCacheDir();
                    DMLog.e('dump_dex_common',"files dir: " + filesDir);
                    DMLog.e('dump_dex_common',"code cache dir: " + codeCacheDir);
                    if (classloader != undefined) {
                        ...
                        fout.close();
                        DMLog.i('dump_dex_common',"write dex to " + dexpath);
                        ...
                        DMLog.i('dump_dex_common',dyndex.toString());
                        ...
                        for (var i in methods) {
                            DMLog.i('dump_dex_common',methods[i].getName());
                            if (methods[i].getName() == "loadAllClass") {
                                DMLog.i('dump_dex_common',"find loadAllClass");
                        ...
                        if (loadAllClass != undefined) {
                            ...
                            DMLog.i('dump_dex_common',"loadAllClass: " + loadAllClass.toString());
                            var args = Java.array('Ljava.lang.Object;', [classloader]);
                            var classlist = loadAllClass.invoke(null, args);
                            DMLog.i('dump_dex_common',"start dump dex ");
                            for (var i in dexrec) {
                                if (Memory.readU32(dexrec[i]) == DEX_MAGIC) {
                                    ...
                                    DMLog.i('dump_dex_common',dumppath);
                                    var dumpdexfile = new File(dumppath, "wb");
                                    dumpdexfile.write(Memory.readByteArray(dexrec[i], dex_len));
                                    dumpdexfile.close();
                                    DMLog.i('dump_dex_common',"write file to " + dumppath);
                                    DMLog.i('dump_dex_common', dumppath);
                                }
                            }
                    
                            DMLog.i('dump_dex_common',"End dump dex ");
                        }
                    } else {
                        DMLog.e('dump_dex_common',"unable get classloader");
                    }
                    return result;
                }
            } else {
                DMLog.e('dump_dex_common',"application is null");
            }
        });
    }
}
